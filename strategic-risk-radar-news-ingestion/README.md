...
Guardian joins the same terms with plain `OR` and lets the HTTP client URL
encode spaces and operators, for example `military escalation OR conflict
escalation`.
...
- Run the `Project Keyword Groups` folder to test the same term groups used by
  the application.
