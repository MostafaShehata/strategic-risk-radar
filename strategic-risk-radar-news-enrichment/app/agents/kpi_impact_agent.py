...
class KpiImpactAgent(BaseAgent):
    def __call__(self, state: EnrichmentState) -> EnrichmentState:
        ...
        llm_result = self.topic_ollama.json_task(
            ...
            "Create KPI names that are useful for UAE identity, citizenship, ports, cargo, border security, visas, "
            "passenger movement, and decision-maker situational awareness. Return an empty array if no KPI is affected.",
            text[:6000],
        )
