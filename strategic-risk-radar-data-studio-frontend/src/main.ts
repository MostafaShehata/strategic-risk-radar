...
@Component({
  ...
  template: `
      ...
      <aside class="sidebar">
        <div class="brand">
          ...
          <div>
            ...
            <small>Operational data viewer</small>
          </div>
        </div>
        <button *ngFor="let item of tabs" [class.active]="tab===item" (click)="tab=item">{{item}}</button>
        <div class="system-card">
          <small>RAG status</small>
          <strong>{{ragStatus?.status || 'loading'}}</strong>
          <span>{{ragStatus?.points_count || 0}} indexed chunks</span>
        </div>
      </aside>
    ...
    <main class="studio-main">
      <header>
        <div>
          <span class="eyebrow">DATA STUDIO</span>
          <h1>Strategic Risk Radar</h1>
          <p>Browse ingestion runs, enrichment runs, raw documents, enriched records, topics, KPI impact, and RAG status.</p>
        </div>
